...
/*
            if (neighbour_props.opaque) {
                //cm_lighting_touch_block(chunk_manager *cm, vec3l pos)

                // without this line everywhere has 0 light?
                world_set_illumination(cm, neighbour_pos, current_illumination - 1); // this is to re if its another chunk
                continue;
            }
*/
...
void cm_add_light(chunk_manager *cm, uint8_t luminance, long x, long y, long z) {
    ...
    printf("add light: setting illumination at %ld %ld %ld to %u\n", x, y, z, luminance);
...
void cm_update_light_for_block_placement(chunk_manager *cm, long x, long y, long z) {
    ...
    //for (direction dir = 0; dir < NUM_DIRS; dir++) {
        //vec3l neighbour_pos = vec3l_add(placed_pos, unit_vec3l[dir]);
    //}
    finish_deleting(cm);
    finish_propagating(cm);
}
...
void cm_update_light_for_block_deletion(chunk_manager *cm, long x, long y, long z) {
    /*
    vec3l new_light_pos = {x,y,z};
    printf("light level here is %u\n", world_get_illumination(cm, new_light_pos));
    vec3l_queue_push(&propagation_queue, new_light_pos);
    finish_propagating(cm);
*/
